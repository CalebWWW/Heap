...
public class Heap<E> {
    ...
    public void decreaseKeyAt(int i) {
    	boolean hasLeft = 2 * i + 1 < heapSize;
    	boolean hasRight = 2 * i + 2 < heapSize;

    	if (hasRight && hasLeft) { 
    		if (0 < compy.compare(internal[left(i)], internal[i])) {
    			if (0 < compy.compare(internal[right(i)], internal[i]) && 0 < compy.compare(internal[right(i)], internal[left(i)])) {
    				swap(right(i), i);
    		...
    		if (0 < compy.compare(internal[right(i)], internal[i])) {
    			swap(right(i), i);
    			decreaseKeyAt(right(i));
    		}
    	}
    	else if (hasRight && hasLeft == false && 0 < compy.compare(internal[i], internal[right(i)])) { 
    		swap(right(i), i);
    		decreaseKeyAt(right(i));
    	}
    	else if (hasLeft && hasRight == false && 0 > compy.compare(internal[i], internal[left(i)])) {
    		swap(left(i), i);
    		decreaseKeyAt(left(i));
    	}
    }
    ...
    public void increaseKeyAt(int i) {
    	if (i < heapSize && i > 0 && 0 < compy.compare(internal[i], internal[parent(i)])) {
    		swap(i, parent(i));
